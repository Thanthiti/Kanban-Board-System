...
async function getOwnerBoards(userId) {
  try {
    ...
    if (!board || board.length === 0) {
      ...
      logger.info(
        `Board ${userId} has ${board.length} boards for user ${userId}`
      );
    }
...
async function getMembers(boardId, userId) {
  try {
    const board = await boardRepository.findById(boardId);
    if (!board) {
      logger.error(
        `Board retrieval failed - Board not found in DB - ${boardId}`
      );
      throw new Error("Board not found in DB");
    }
    ...
    if (!members || members.length === 0) {
      ...
      logger.info(
        `Board ${boardId} has ${members.length} members for board ${boardId}`
      );
    }
